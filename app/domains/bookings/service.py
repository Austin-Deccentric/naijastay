...
def _as_utc(value: datetime) -> datetime:
    # Postgres may return naive datetimes; compare safely either way.
    return value if value.tzinfo is not None else value.replace(tzinfo=UTC)


async def create_booking(
    session: AsyncSession, data: BookingCreate, client: User
) -> Booking:
   
    if client.role == UserRole.GUEST:
        guest_email = client.email  
    else:
        #verify reception provided guest email
    ...
    # Room Check
    try:
        room = await get_room(data.room_id, session)
    ...
    if hold.guest_email.strip().lower() != guest_email:
        raise HoldMismatch("This hold belongs to a different guest.")
    ...
    # Check if other nights are taken
    taken = await nights_taken(session, data.room_id, data.check_in, data.check_out)
    if taken:
