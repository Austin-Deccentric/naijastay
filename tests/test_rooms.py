...
async def test_get_all_rooms_lists_seeded_rooms(client):
...
async def test_get_all_rooms_open_without_auth(client):
    # No auth dependency by design (dashboard + SSE clients).
    assert (await client.get("/api/v1/rooms/")).status_code == 200


async def test_get_all_rooms_room_state_filter(client):
    await helpers.set_room_state(101, "dirty")
    clean = await client.get("/api/v1/rooms/", params={"room_state": "clean"})
    assert clean.status_code == 200, clean.text
    assert all(r["room_state"] == "clean" for r in clean.json())
    assert all(r["room_id"] != 101 for r in clean.json())
    dirty = await client.get("/api/v1/rooms/", params={"room_state": "dirty"})
    assert dirty.status_code == 200, dirty.text
    assert [r["room_id"] for r in dirty.json()] == [101]


async def test_get_all_rooms_invalid_room_state_returns_422(client):
    assert (await client.get("/api/v1/rooms/", params={"room_state": "muddy"})).status_code == 422


async def test_search_returns_free_standard_room(client, guest_headers):
...
async def test_search_rejects_bad_dates(client, guest_headers):
...
async def test_search_excludes_unavailable_room(client, guest_headers):
...
async def test_receptionist_can_list_rooms(client, receptionist_headers):
    resp = await client.get("/api/v1/rooms/", headers=receptionist_headers)
    assert resp.status_code == 200, resp.text
